...
void printFlagIntDtt(int totalStation, int totalBus, int** dtt);
...
node_pointer read_lines(char* buffer) {
	...
	// microSeconds,  timezone을 포함하면 strSecond의 구분자를 공백(" ")에서 온점(".")으로 변경하세요.
	char* strSecond = strtok(NULL, " ");				
	int second = atoi(strSecond);

	// 출력양식 수정으로 12월 4일을 포함한 이후 데이터부터는 필요 없습니다. 
	// (microSeconds, timezone을 포함하지 않습니다. [.00000+09:00])
	//char* _ = strtok(NULL, " ");						
	//printf("%s\n", _);
...
/* stationExp보다 "작은" stationSeq가 들어왔을-새로운 버스가 운행을 시작할때-
headC를 업데이트 하고 노드를 추가한다.*/
void updateHeadC(headc_pointer headC, headp_pointer headP, node_pointer node) {
...
void writeDailyTimeTable(node_pointer** dtt, headr_pointer headR, headp_pointer headP, filepath_pointer file, routeId_pointer tempRouteId) {
	...
	int mkResult = _mkdir(outputPath);
	...
	int col = headP->totalBus;
	fprintf(outputFile, "%d ", tempRouteId->routeId);
...
void sumFunction2(filepath_pointer outputFile, headd_pointer topD)
{
	...
	fscanf(inputFile, "%d %d %d", &routeId, &totalStation, &totalBus);
	...
	//printIntDtt(totalStation, totalBus, dttPointer->dailyTimeTable);																								//일일시간표 출력 dailyTimeTable
	printFlagIntDtt(totalStation, totalBus, headD->flagTable);																										//flagTable 출력
	//printIntDtt(totalStation, totalBus, headD->sumTable);																												//sumTable 출력
